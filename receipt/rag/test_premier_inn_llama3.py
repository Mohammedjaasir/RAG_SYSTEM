...
Uses llama3 model for better stability.
"""
...
def test_premier_inn_invoice_extraction():
    ...
    with setup_output_capture(__file__):
        print("\n" + "="*70)
        print("PROCESSING PREMIER INN INVOICE (TRANSCRIPTION FROM IMAGE) - USING LLAMA3")
        print("="*70 + "\n")
        
        # Use a temporary database path specifically for this test to avoid corruption
        temp_db = Path(__file__).parent / "temp_chroma_db_llama3"
        if temp_db.exists():
            import shutil
            shutil.rmtree(temp_db)
            
        # Explicitly passing model_name="llama3"
        pipeline = get_rag_pipeline(persist_directory=str(temp_db), model_name="llama3")
        
        print(f"\n[*] Running extraction using model: llama3...")
        result = pipeline.extract_from_ocr(invoice_ocr, retrieve_k=3)
        
        # Display results with the requested header and all details
        print("\n" + "="*70)
        print("RECEIPT RAG EXTRACTION RESULTS")
        print("="*70)
        ...
        pipeline.save_result_to_file(result, "premier_inn_llama3")
